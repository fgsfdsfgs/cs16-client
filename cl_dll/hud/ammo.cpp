...
void WeaponsResource :: LoadWeaponSprites( WEAPON *pWeapon )
{
	...
	if (p)
	{
		...
		gHR.iHistoryGap = mymax( gHR.iHistoryGap, pWeapon->rcActive.bottom - pWeapon->rcActive.top );
	}
	else
		pWeapon->hInactive = 0;
...
int CHudAmmo::VidInit(void)
{
	...
	gHR.iHistoryGap = mymax( gHR.iHistoryGap, gHUD.GetSpriteRect(m_HUD_bucket0).bottom - gHUD.GetSpriteRect(m_HUD_bucket0).top);
...
void CHudAmmo::DrawCrosshair( float flTime )
{
	...
	{
		m_flCrosshairDistance = mymin( m_flCrosshairDistance + iDeltaDistance, 15.0f );
		m_iAlpha = mymax( m_iAlpha - 40, 120 );
	}
	...
	m_flCrosshairDistance = mymax( m_flCrosshairDistance, iDistance );
